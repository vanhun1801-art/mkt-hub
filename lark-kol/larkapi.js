...
async function listAllRecords(tableId = cfg.bang.hopTac, base) {
  const out = [];
  let offset = 0;
 ...
 */
async function getRecord(recordId, tableId = cfg.bang.hopTac) {
  let offset = 0;
  for (let trang = 0; trang < 30; trang++) {
 ...
 */
async function createMany(rows, tableId = cfg.bang.hopTac, base) {
...
async function deleteRecords(recordIds, tableId = cfg.bang.hopTac, base) {
