...
async function listAllRecords(tableId = cfg.dangKyTableId, base) {
  const out = [];
  let offset = 0;
 ...
 */
async function getRecord(recordId, tableId = cfg.dangKyTableId) {
  let offset = 0;
  for (let trang = 0; trang < 30; trang++) {
 ...
 */
async function createMany(rows, tableId = cfg.dangKyTableId, base) {
...
async function deleteRecords(recordIds, tableId = cfg.dangKyTableId, base) {
