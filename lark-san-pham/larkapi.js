...
async function listAllRecords(tableId = cfg.spTableId, base) {
  const out = [];
  let offset = 0;
 ...
 */
async function getRecord(recordId, tableId = cfg.spTableId) {
  let offset = 0;
  for (let trang = 0; trang < 30; trang++) {
 ...
 */
async function createMany(rows, tableId = cfg.spTableId, base) {
...
async function deleteRecords(recordIds, tableId = cfg.spTableId, base) {
