  ...
  return new Promise((resolve, reject) => {
    if (!cfg.cliScript) {
      return reject(new Error(
        'Không tìm thấy lark-cli trên máy này. Cài bằng: npm i -g @larksuite/cli, ' +
...
/** Chế độ cli gửi tin nhắn bằng danh tính bot. */
async function sendMessage(openId, text) {
  try {
    await cli(['im', '+messages-send', '--user-id', openId, '--text', text, '--as', 'bot', '--format', 'json'], { retries: 1 });
    return true;
